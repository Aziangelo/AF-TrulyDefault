$input v_color0, CubePosition, v_fogColor, v_fogControl, TIME, v_cpos
...
vec3 CloudBase(vec3 color, vec3 normalPos) {
  vec3 CloudCol_1; // Day
  ...
  CloudCol_4 = mix (CloudCol_4, vec3(0.6), abs(normalPos.z));
  CloudCol_4 = mix (CloudCol_4, vec3(0.5), (-normalPos.y));

  vec3 CloudF;
  CloudF = timecycle3(CloudCol_1, CloudCol_2, CloudCol_3);
  CloudF = mix (CloudF, mix(CloudCol_4, CloudCol_4*0.7, AFnight), AFrain);
...
//#include <azify/utils/components.glsl> // Components Files
  ...
  albedo.rgb = CloudBase(albedo.rgb, normal);
